    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-blue-900 bg-opacity-80 text-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                @endif
                <h3 class="text-lg font-bold mb-4">Categories Table</h3>
                        ...
                        @foreach ($categories as $category)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button wire:click="edit({{ $category->id }})" class="bg-blue-700 hover:bg-blue-600 text-white font-bold py-1 px-3 rounded">Edit</button>
                                    <button wire:click="delete({{ $category->id }})" class="bg-red-700 hover:bg-red-600 text-white font-bold py-1 px-3 rounded ml-2">Delete</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                <h3 class="text-lg font-bold mb-4">@if($editMode) Edit Category @else Create Category @endif</h3>
                <form wire:submit.prevent="@if($editMode) update @else create @endif" class="space-y-4">
                    <div>
                        <label for="name" class="block text-sm font-medium">Name</label>
                        <input wire:model.defer="name" id="name" type="text" class="mt-1 block w-full rounded-md bg-blue-800 border border-blue-700 text-white" required />
